...
export default function Report() {
    ...
    const handlePrint = async () => {
        try {
            ...
            // FIX: html2canvas crashes on lab() or oklch() colors generated by Tailwind v4.
            // It parses ALL stylesheets in the document (even unused ones).
            // We sweep all <style> tags and inline local <link> sheets, converting their oklch/lab colors to rgba() 
            // before running the generator, then restore them after.
            ...
            const getRgbFromColor = (colorStr: string) => {
                ...
                if (!ctx) {
                    const canvas = document.createElement('canvas'); // Use real document canvas
                    canvas.width = 1;
                    canvas.height = 1;
            ...
            // 1. Sanitize local <link> stylesheets (convert to <style>)
                ...
                try {
                    ...
                    if (href && href.startsWith(window.location.origin)) {
                        ...
                        if (css.includes('oklch(') || css.includes('lab(') || css.includes('color(')) {
                            ...
                            link.remove(); // Temporarily hide link
            ...
            // 3. Sanitize inline explicit styles on the elements (retained for completeness)
            ...
            elements.forEach((el: Element) => {
                ...
                for (let i = 0; i < style.length; i++) {
                    ...
                    if (val && (val.includes('lab(') || val.includes('oklch(') || val.includes('color('))) {
                        changes[prop] = htmlEl.style.getPropertyValue(prop); // save raw inline
